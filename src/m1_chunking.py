...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        cur = ""
        for word in words:
            if cur and len(cur) + len(word) + 1 > child_size:
                children.append(Chunk(text=cur.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                cur = ""
            cur += word + " "
        if cur.strip():
            children.append(Chunk(text=cur.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
        ...
        if body:
            chunks.append(Chunk(
                ...
                metadata={**metadata, "chunk_index": len(chunks),
                          "section": current_header.lstrip("#").strip(), "strategy": "structure"},
            ))
